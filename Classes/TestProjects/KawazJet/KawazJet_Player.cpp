...
bool KawazJetPlayer::init()
{
    ...
    // カテゴリをPLAYERにセットする
    body->setCategoryBitmask(static_cast<int>(KawazJetStage::TileType::PLAYER));
    // 壁とのみ衝突する
    body->setCollisionBitmask(static_cast<int>(KawazJetStage::TileType::WALL));
    // 全ての剛体と接触判定を行う
...
void KawazJetPlayer::update(float dt)
{
    // 毎フレームかわずたんに推進力を与える
