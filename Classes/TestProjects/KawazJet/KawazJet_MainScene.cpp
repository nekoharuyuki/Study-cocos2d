...
Scene* KawazJetMainScene::createSceneWithStage(int level)
{
    ...
    auto layer = new KawazJetMainScene();
...
bool KawazJetMainScene::init()
{
    return this->initWithLevel(0);
}

bool KawazJetMainScene::initWithLevel(int level)
{
    if (!Layer::init()) {
        return false;
    }
    ...
    auto stage = KawazJetStage::createWithLevel(level);
    this->setStage(stage);
    ...
    contactListener->onContactBegin = [this](PhysicsContact& contact) {
        ...
        if (category & static_cast<int>(KawazJetStage::TileType::ENEMY)) {
            // ゲームオーバー
            this->onGameOver();
        } else if (category & (int)KawazJetStage::TileType::COIN) {
            // コイン
            ...
            _coin += 1;
        } else if (category & static_cast<int>(KawazJetStage::TileType::ITEN)) {
            // アイテム
            layer->removeChild(body->getNode(), true);
...
KawazJetMainScene::KawazJetMainScene()
: _isPress(false)
, _coin(0)
, _itemCount(0)
, _second(0)
...
void KawazJetMainScene::onEnterTransitionDidFinish()
{
...
void KawazJetMainScene::update(float dt)
{
    // 背景をプレイヤーの位置によって動かす
...
void KawazJetMainScene::onGameOver()
{
    ...
    auto menuItem = MenuItemImage::create("replay.png", "replay_pressed.png", [currentStage](Ref *sender) {
        ...
        auto scene = KawazJetMainScene::createSceneWithStage(currentStage);
...
void KawazJetMainScene::onClear()
{
    auto winSize = Director::getInstance()->getWinSize();
    ...
    auto menuItem = MenuItemImage::create("next.png", "next_pressed.png", [nextStage](Ref *sender) {
        //CocosDenshion::SimpleAudioEngine::getInstance()->playEffect(AudioUtils::getFileName("decide").c_str());
        auto scene = KawazJetMainScene::createSceneWithStage(nextStage);
        auto transition = TransitionFade::create(1.0, scene);
        Director::getInstance()->replaceScene(transition);
    });
    auto returnTitle = MenuItemImage::create("return.png", "return_pressed.png", [](Ref *sender) {
...
void KawazJetMainScene::onGetItem(cocos2d::Node * item)
{
    _itemCount += 1;
...
void KawazJetMainScene::updateSecond()
{
